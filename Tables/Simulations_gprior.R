...
G <- c(1, 10) #prior parameter g
c <- 0.001 #prior parameter c

#options
L_val <- c(200, 200, 200) #lag
max_iter <- 1000 #maximum number of iterations for the coupling
  ...
  for(k in 1:length(P)){
    ...
    for(r in 1:replicas){
      ...
      if(print_message & r %% len_to_print == 0){
        print(paste("Replica", r, "with n =", n, "p =", p, "and", "g =", g))
...
#saveRDS(meeting_DA, "meeting_DA_gprior_imb.rds")
#saveRDS(meeting_GS, "meeting_GS_gprior_imb.rds")
#saveRDS(meeting_DA, "meeting_DA_gprior_average.rds")
#saveRDS(meeting_GS, "meeting_GS_gprior_average.rds")
...
#g = 1
ind_g <- 1
perf_DA1 <- meeting_DA[[ind_g]][,1]
perf_GS1 <- meeting_GS[[ind_g]][,1]
perf_DA2 <- meeting_DA[[ind_g]][,2]
perf_GS2 <- meeting_GS[[ind_g]][,2]
perf_DA3 <- meeting_DA[[ind_g]][,3]
perf_GS3 <- meeting_GS[[ind_g]][,3]
